...
def update_list_by_key(
    ...
    Args:
        ...
        lookup_key (str): key to use for looking up dictionaries
        callback (Callable[[Any, Any, Dict[str, Any], str], Any]): callback function to use for updating dictionaries.
            Takes the old value, new value, additional arguments, and default key as input and returns the updated value
        default (Dict[str, Any], optional): default dictionary to use for updating
        default_key (str, optional): key to use for looking up the default dictionary
    ...
    for key in keys:
        ...
        _new: Any = callback(old_obj.get(key, {}), update, {}, default_key)
        old_obj[key] = _new
